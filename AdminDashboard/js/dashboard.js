(function () {
  ...
  function openAddAnimeModal() { const modal = document.getElementById('add-anime-modal'); modal.hidden = false; modal.setAttribute('aria-hidden', 'false'); document.getElementById('kitsu-search-input').focus(); }
  function closeAddAnimeModal() { const modal = document.getElementById('add-anime-modal'); modal.hidden = true; modal.setAttribute('aria-hidden', 'true'); }
  function setAnimeImportTab(name) { document.querySelectorAll('[data-anime-tab]').forEach(tab => tab.setAttribute('aria-selected', String(tab.dataset.animeTab === name))); document.querySelectorAll('[data-anime-panel]').forEach(panel => { panel.hidden = panel.dataset.animePanel !== name; }); }
  function renderKitsuResults(results) { const container = document.getElementById('kitsu-search-results'); if (!results.length) { container.innerHTML = '<p class="item-sub">No Kitsu results found.</p>'; return; } container.innerHTML = results.map(anime => `<article class="kitsu-result"><img src="${esc(anime.cover_image || '')}" alt="" onerror="this.style.visibility='hidden'"><div><h4>${esc(anime.title)}</h4><p>${esc(anime.year || 'Unknown year')} · ${esc(anime.episode_count || '?')} episodes</p><p class="synopsis">${esc(anime.description || 'No synopsis available.')}</p></div><button class="btn" type="button" data-import-kitsu-id="${esc(anime.id)}">Import</button></article>`).join(''); }
  async function searchKitsu(event) { event.preventDefault(); const form = event.currentTarget; const query = form.querySelector('input').value.trim(); if (!query) return; const submit = form.querySelector('[type=submit]'); const container = document.getElementById('kitsu-search-results'); submit.disabled = true; submit.textContent = 'Searching...'; container.innerHTML = '<p class="item-sub">Searching Kitsu…</p>'; try { const results = await window.apiRequest(`/anime/search?q=${encodeURIComponent(query)}`); renderKitsuResults(Array.isArray(results) ? results : []); } catch (error) { container.innerHTML = `<p class="item-sub">${esc(error.message || 'Search failed.')}</p>`; } finally { submit.disabled = false; submit.textContent = 'Search'; } }
  async function importKitsuAnime(button) { const kitsuId = button.dataset.importKitsuId; button.disabled = true; button.textContent = 'Importing...'; try { await window.apiRequest('/admin/import-anime', { method: 'POST', body: { kitsuId } }); closeAddAnimeModal(); await loadAnime(); await loadOverview(); /* Existing table refresh occurs above. */ } catch (error) { alert(`Anime was not imported: ${error.message}`); button.disabled = false; button.textContent = 'Import'; } }
  async function createManualAnime(event) { event.preventDefault(); const form = event.currentTarget; const submit = form.querySelector('[type=submit]'); submit.disabled = true; submit.textContent = 'Creating...'; try { const data = Object.fromEntries(new FormData(form)); data.is_premium = form.is_premium.checked; data.is_featured = form.is_featured.checked; if (form.cover_file.files[0]) data.cover_image = (await uploadImage(form.cover_file.files[0], 'anime')).url; if (form.banner_file.files[0]) data.banner_image = (await uploadImage(form.banner_file.files[0], 'banners')).url; delete data.cover_file; delete data.banner_file; await window.apiRequest('/admin/anime', { method: 'POST', body: data }); closeAddAnimeModal(); form.reset(); await loadAnime(); await loadOverview(); /* Existing table refresh occurs above. */ } catch (error) { alert(`Anime was not created: ${error.message}`); } finally { submit.disabled = false; submit.textContent = 'Create Anime'; } }
