(function () {
  ...
  async function openAnimeEditor(id = null) {
    ...
    modal.querySelector('#anime-form').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = Object.fromEntries(new FormData(form)); data.is_premium = form.is_premium.checked; data.is_featured = form.is_featured.checked; if (form.cover_file.files[0]) data.cover_image = await uploadImage(form.cover_file.files[0], 'anime'); if (form.banner_file.files[0]) data.banner_image = await uploadImage(form.banner_file.files[0], 'banners'); delete data.cover_file; delete data.banner_file; await window.apiRequest(id ? `/admin/anime/${id}` : '/admin/anime', { method: id ? 'PUT' : 'POST', body: data }); modal.remove(); await loadAnime(); await loadOverview(); });
  }
  ...
  async function openEpisodeEditor() {
    ...
    modal.querySelector('#episode-form').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = Object.fromEntries(new FormData(form)); data.is_premium = form.is_premium.checked; if (form.thumbnail_file.files[0]) data.thumbnail_url = await uploadImage(form.thumbnail_file.files[0], 'thumbnails'); delete data.thumbnail_file; const animeId = data.anime_id; delete data.anime_id; await window.apiRequest(`/admin/anime/${animeId}/episodes`, { method: 'POST', body: data }); modal.remove(); await loadEpisodes(); await loadOverview(); });
  }
  ...
  async function openEpisodeEdit(id) {
    ...
    modal.querySelector('#episode-edit-form').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = Object.fromEntries(new FormData(form)); data.is_premium = form.is_premium.checked; await window.apiRequest(`/admin/episodes/${id}`, { method: 'PUT', body: data }); modal.remove(); await loadEpisodes(); await loadOverview(); });
  }
