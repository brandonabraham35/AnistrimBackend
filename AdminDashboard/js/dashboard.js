(function () {
  ...
  async function loadOverview() {
    try {
      const [data, paymentData] = await Promise.all([
        window.apiRequest('/admin/dashboard/overview'),
        window.apiRequest('/payments/revenue'),
      ]);
  ...
  function showSection(section) {
    ...
    setText('page-title', ({ dashboard: 'Administrative Overview', anime: 'Anime List', users: 'Users Management', payments: 'Payments' })[section]);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    showSection(['dashboard', 'anime', 'users', 'payments'].includes(location.hash.slice(1)) ? location.hash.slice(1) : 'dashboard');
