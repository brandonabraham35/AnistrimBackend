    ...
    _users_tableBody.innerHTML = pageItems.map(user => {
      ...
      return `<tr>
        ...
        <td style="white-space:nowrap;">
          <button class="btn-action edit" data-id="${user.id}" title="Edit User" aria-label="Edit User"><i class="fas fa-edit"></i> Edit</button>
          <button class="btn-action ban" data-id="${user.id}" data-status="${user.status}" title="${user.status === 'banned' ? 'Unban User' : 'Ban User'}" aria-label="${user.status === 'banned' ? 'Unban User' : 'Ban User'}">
            ${user.status === 'banned' ? '<i class="fas fa-check-circle" style="color:var(--success);"></i> <span>Unban</span>' : '<i class="fas fa-ban" style="color:var(--warning);"></i> <span>Ban</span>'}
          </button>
          <button class="btn-action delete" data-id="${user.id}" title="Delete User" aria-label="Delete User"><i class="fas fa-trash"></i> Delete</button>
        </td>
      </tr>`;
    }).join('');
