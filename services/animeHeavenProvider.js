...
/**
 * Perform a lightweight HEAD request to verify if a source URL is live.
 * Returns a structured object with the outcome.
 * @param {object} source - The source object, containing url, referer, etc.
 * @returns {Promise<{live: boolean, status: number, reason: string|null, durationMs: number}>}
 */
async function verifySourceLiveness(source) {
  const url = source?.url;
  if (!url || !isPlayableMediaUrl(url)) {
    return { live: false, status: 0, reason: 'invalid_url', durationMs: 0 };
  }

  const started = Date.now();
  const context = getPlaybackContext(url, source.referer);
  const headers = {
  ...
  try {
    const res = await request(
      ...
      {
        providerName: `${PROVIDER_NAME}-liveness`,
        streaming: true,
        skipProxy: true,
        dontTrackHealth: true,
        timeout: 4000,
    ...
    return { live: isLive, status: res.status, reason: isLive ? null : 'non_2xx_3xx_status', durationMs };
  } catch (err) {
    const durationMs = Date.now() - started;
    const status = Number(err?.response?.status || 0);
    const reason = (status === 403 || status === 404) ? 'cdn_rejected' : (err.code || 'network_error');
    return { live: false, status, reason, durationMs };
  }
}
...
async searchAnime(query, limit = 10, episode) {
    logger.debugStream('[AnimeHeaven] searchAnime started', { query });
    const q = String(query || '').trim();
    if (!q) return [];
    ...
    try {
      ...
      logger.debugStream('[AnimeHeaven] Calling pickBaseUrl...', { query });
...
let targetIdentifier = identifier || slug || null;
  ...
  async resolvePlayer({ title, episode, identifier, slug }) {
    logger.debugStream('[AnimeHeaven] resolvePlayer started', { title, episode });
    const started = Date.now();
    const timings = {};
    try {
...
// Attach the full playback context (referer + origin + cookies) to each
      ...
      timings.total = Date.now() - started; // Ensure total is logged even on failure
