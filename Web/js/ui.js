...
(function () {
  ...
  function renderHeader() {
    ...
    h.innerHTML = '<nav class="nav"><div class="nav-inner">' +
      ...
      (logged ? '<a href="#/upgrade">Upgrade</a>' : '') +
      '</div>' +
      '<div class="nav-search"><input type="text" placeholder="Search anime\u2026" id="nav-search-input" onkeydown="if(event.key===\'Enter\'){var q=this.value.trim();if(q)window.AniStrimRouter.navigate(\'/search\',{q:q})}"></div>' +
      '<div class="nav-auth">' +
      (logged
      ...
      (logged && user ? '<div class="nav-user">' +
        ...
        (user.email ? '<div style="font-size:var(--font-sm);color:var(--text-muted)">' + esc(user.email) + '</div>' : '') +
        '</div></div>' : '') +
      '<a href="#/">Home</a><a href="#/browse">Browse</a><a href="#/search">Search</a>' +
      (logged ? '<a href="#/watchlist">Watchlist</a><a href="#/history">History</a>' : '') +
      '<a href="#/upgrade">Upgrade</a>' +
      (logged ? '<a href="#/profile">Profile</a><a href="#/settings">Settings</a><a href="#" onclick="AniStrimUI.logout();AniStrimUI.closeMobileNav();return false">Logout</a>' : '<a href="#/login">Sign In</a><a href="#/signup">Sign Up</a>') +
      '</div></nav>';
